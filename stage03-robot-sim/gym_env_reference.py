...
ENV_NAME = "CartPole-v1"                            #？该强化学习环境的作用
...
def reset_env(env):                                 #？这一步的作用是什么结合gymnasium.error.ResetNeeded: Cannot call env.step() before calling env.reset()
...
def step_env(env, action):
    ...
    return next_observation, reward, terminated, truncated, info #？各个变量含义
...
def print_step_result(observation, action, next_observation, reward, terminated, truncated):
    ...
    print(f"reward: {reward}")                          #？reward的作用或者后续作用
